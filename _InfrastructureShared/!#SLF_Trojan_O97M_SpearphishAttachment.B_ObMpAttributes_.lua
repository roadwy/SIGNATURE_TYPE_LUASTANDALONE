local L0_0, L1_1
L0_0 = mp
L0_0 = L0_0.getfilename
L1_1 = mp
L1_1 = L1_1.bitor
L1_1 = L1_1(mp.bitor(mp.FILEPATH_QUERY_FNAME, mp.FILEPATH_QUERY_PATH), mp.FILEPATH_QUERY_LOWERCASE)
L1_1 = L0_0(L1_1, L1_1(mp.bitor(mp.FILEPATH_QUERY_FNAME, mp.FILEPATH_QUERY_PATH), mp.FILEPATH_QUERY_LOWERCASE))
if L0_0 == nil or #L0_0 < 40 or L1_1 == nil then
  return mp.CLEAN
end
if string.find(L1_1, "->", 1, true) == nil then
  return mp.CLEAN
end
if string.find(L0_0, "newtonsoft.json", 1, true) ~= nil or string.find(L1_1, "newtonsoft.json", 1, true) ~= nil then
  return mp.CLEAN
end
if 1 ~= nil then
  if string.sub(L1_1, #L1_1 - 3, #L1_1 - 3) == "." then
    if string.sub(L1_1, -3) ~= nil and ({
      vbs = true,
      vbe = true,
      lnk = true,
      bat = true,
      cmd = true,
      js = true,
      hta = true,
      exe = true,
      wsh = true,
      ps1 = true,
      jse = true
    })[string.sub(L1_1, -3)] == true then
      return mp.INFECTED
    end
    return mp.INFECTED
  elseif string.sub(L1_1, #L1_1 - 2, #L1_1 - 2) == "." and string.sub(L1_1, -2) ~= nil and ({
    vbs = true,
    vbe = true,
    lnk = true,
    bat = true,
    cmd = true,
    js = true,
    hta = true,
    exe = true,
    wsh = true,
    ps1 = true,
    jse = true
  })[string.sub(L1_1, -2)] == true then
    return mp.INFECTED
  end
end
return mp.CLEAN

...
L2_2 = string.gsub(L2_2, "\"\"", "\"")
if string.match(L2_2, "stop[%s]*\"") then
  L3_3 = string.match(L2_2, "stop[%s]*\"([^\"]+)\"[%s]*")
else
  L3_3 = string.match(L2_2, "stop[%s]*(%S+)[%s]*")
end
if L3_3 == "" or L3_3 == nil then
  return mp.CLEAN
end
if ({hpaudioanalytics = true, dellclientmanagementservice = true})[L3_3] then
  return mp.CLEAN
end

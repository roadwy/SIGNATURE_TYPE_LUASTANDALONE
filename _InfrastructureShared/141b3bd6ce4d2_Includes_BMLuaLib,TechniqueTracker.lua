-- Decompiled using luadec 2.2 rev: 895d923 for Lua 5.1 from https://github.com/viruscamp/luadec
-- Command line: /mnt/d/out/_InfrastructureShared/141b3bd6ce4d2_Includes_BMLuaLib,TechniqueTracker 

-- params : ...
-- function num : 0
-- DECOMPILER ERROR at PC16: Overwrote pending register: R0 in 'AssignReg'

do
  if (this_sigattrlog[1]).matched and (this_sigattrlog[1]).wp2 ~= nil then
    local l_0_0 = nil
  else
  end
  -- DECOMPILER ERROR at PC52: Overwrote pending register: R0 in 'AssignReg'

  do
    if (not (this_sigattrlog[2]).matched or (this_sigattrlog[2]).wp2 == nil or (this_sigattrlog[3]).matched) and (this_sigattrlog[3]).wp2 ~= nil then
      local l_0_1 = (string.lower)((this_sigattrlog[2]).utf8p2)
    else
    end
    -- DECOMPILER ERROR at PC88: Overwrote pending register: R0 in 'AssignReg'

    do
      if (not (this_sigattrlog[4]).matched or (this_sigattrlog[4]).wp2 == nil or (this_sigattrlog[5]).matched) and (this_sigattrlog[5]).wp2 ~= nil then
        local l_0_2 = (string.lower)((this_sigattrlog[4]).utf8p2)
      else
      end
      -- DECOMPILER ERROR at PC124: Overwrote pending register: R0 in 'AssignReg'

      do
        if (not (this_sigattrlog[6]).matched or (this_sigattrlog[6]).wp2 == nil or (this_sigattrlog[7]).matched) and (this_sigattrlog[7]).wp2 ~= nil then
          local l_0_3, l_0_4, l_0_5 = (string.lower)((this_sigattrlog[6]).utf8p2)
        end
        -- DECOMPILER ERROR at PC125: Confused about usage of register: R0 in 'UnsetPending'

        -- DECOMPILER ERROR at PC134: Confused about usage of register: R0 in 'UnsetPending'

        -- DECOMPILER ERROR at PC143: Confused about usage of register: R0 in 'UnsetPending'

        if l_0_3 and (this_sigattrlog[2]).matched and (string.find)(l_0_3, "copy-as-is", 1, true) and (string.find)(l_0_3, "/usr/share/rear", 1, true) then
          return mp.CLEAN
        end
        local l_0_6 = nil
        -- DECOMPILER ERROR at PC170: Overwrote pending register: R1 in 'AssignReg'

        -- DECOMPILER ERROR at PC188: Overwrote pending register: R1 in 'AssignReg'

        -- DECOMPILER ERROR at PC193: Confused about usage of register: R1 in 'UnsetPending'

        -- DECOMPILER ERROR at PC200: Confused about usage of register: R1 in 'UnsetPending'

        -- DECOMPILER ERROR at PC209: Confused about usage of register: R1 in 'UnsetPending'

        -- DECOMPILER ERROR at PC218: Confused about usage of register: R1 in 'UnsetPending'

        -- DECOMPILER ERROR at PC227: Confused about usage of register: R1 in 'UnsetPending'

        -- DECOMPILER ERROR at PC236: Confused about usage of register: R1 in 'UnsetPending'

        -- DECOMPILER ERROR at PC245: Confused about usage of register: R1 in 'UnsetPending'

        -- DECOMPILER ERROR at PC256: Unhandled construct in 'MakeBoolean' P3

        if (((this_sigattrlog[9]).matched and (this_sigattrlog[9]).wp2 ~= nil and not (this_sigattrlog[10]).matched) or nil) and ((string.match)(nil, "%d+%.%d+%.%d+%.%d+") or (string.find)(nil, "bit.ly", 1, true) or (string.find)(nil, "goo.gl", 1, true) or (string.find)(nil, "telegram", 1, true) or (string.find)(nil, "dropbox", 1, true) or (string.find)(nil, "api.", 1, true) or (string.find)(nil, "ftp", 1, true)) then
          TrackPidAndTechniqueBM("BM", "T1048", "Exfiltration_Archive")
          addRelatedProcess()
          reportRelatedBmHits()
          RemediateProcessTreeForLinux()
          return mp.INFECTED
        end
        return mp.CLEAN
      end
    end
  end
end


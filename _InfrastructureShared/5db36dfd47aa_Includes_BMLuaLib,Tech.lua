...
-- Command line: /mnt/d/out/_InfrastructureShared/5db36dfd47aa_Includes_BMLuaLib,Tech 
...
do
  ...
  return l_1_1
    ...
    local l_0_5 = nil
    if not IsProcNameInParentProcessTree("BM", {"winword.exe", "mspub.exe", "excel.exe", "powerpnt.exe", "powershell.exe", "cscript.exe", "wscript.exe", "iexplore.exe", "microsoftedge.exe", "microsoftedgecp.exe", "chrome.exe", "firefox.exe", "python.exe", "rundll32.exe", "regsvr32.exe", "psexesvc.exe", "psexec.exe", "services.exe", "wininit.exe", "wmic.exe", "w3wp.exe", "cmd.exe"}) then
      return mp.CLEAN
    end
    local l_0_6 = nil
    ...
    do
      ...
      if l_0_15 == "/transfer" then
        ...
        -- DECOMPILER ERROR at PC150: Overwrote pending register: R6 in 'AssignReg'

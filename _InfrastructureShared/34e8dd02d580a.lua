...
if mp.getfilesize() > 800000 or mp.getfilesize() < 4000 then
  return mp.CLEAN
end
...
if mp.getfilename(mp.bitor(mp.FILEPATH_QUERY_FULL, mp.FILEPATH_QUERY_LOWERCASE)):find("\\libiphoneaccess", 1, true) then
  return mp.CLEAN
end
...
if mp.getfilename(mp.bitor(mp.FILEPATH_QUERY_FULL, mp.FILEPATH_QUERY_LOWERCASE)):find("iosdevicemanagerex", 1, true) then
  return mp.CLEAN
end
return mp.INFECTED
